...
object AutoGenerateDocumentTitle {
    ...
    fun generateTitleForBehovsmelding(behovsmelding: JsonNode): String {
        val behovsmeldingType: Behovsmeldingtype = behovsmelding["behovsmeldingType"]?.asObject() ?: Behovsmeldingtype.SØKNAD

        if (Configuration.application.profile == Profile.LOCAL) {
            return defaultTitle(behovsmeldingType)
        }

        val hjelpemidlerListe = behovsmelding["soknad"]["hjelpemidler"]["hjelpemiddelListe"]
        val hmsnrs = hjelpemidlerListe.mapNotNull { it["hmsNr"].asText() }.filter { it.isNotEmpty() }.toSet()

        return generateTitle(hmsnrs)
    }

    fun generateTitleForBrukerpassbytte(brukerpassbytte: JsonNode): String {
        if (Configuration.application.profile == Profile.LOCAL) {
            return defaultTitle(Behovsmeldingtype.BYTTE)
        }
        val hmsnr = brukerpassbytte["hjelpemiddel"]["artnr"].textValue()
        return generateTitle(setOf(hmsnr))
    }

    private fun generateTitle(hmsnrs: Set<String>): String {
        ...
        isokategorierMap.forEach { (hmsnr, old) ->
            ...
            if (new == null) {
                ...
                unexpectedIsoHmsnrs[hmsnr] = Pair(old.isokortnavn.toString(), new.isoCategoryTextShort.toString())
            } else {
        ...
        if (unexpectedIsoHmsnrs.isNotEmpty()) {
            log.info("DEBUG GRUNNDATA: new dataset has mismatching isoCategoryTextShorts: $unexpectedIsoHmsnrs")
        }
        if (matchesHmsnrs.isNotEmpty()) {
            log.info("DEBUG GRUNNDATA: new dataset matches old for hmsnrs/isoCategoryTextShorts: $matchesHmsnrs")
        }
