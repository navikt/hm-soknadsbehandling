...
object AutoGenerateDocumentTitle {
    ...
    fun generateTitle(behovsmelding: JsonNode): String {
        ...
        if (hmsnrs.isEmpty()) {
            val tilbehørHmsnrs = behovsmelding["soknad"]["hjelpemidler"]["tilbehørListe"]
                .mapNotNull { it["hmsnr"].asText() }
                .filter { it.isNotEmpty() }.toSet()
            if (tilbehørHmsnrs.isNotEmpty()) {
                return defaultTitleKunTilbehør(behovsmeldingType)
            }
        }
    ...
    private fun defaultTitleKunTilbehør(behovsmeldingType: BehovsmeldingType) =
        when (behovsmeldingType) {
            BehovsmeldingType.BESTILLING -> "Bestilling av tilbehør"
            BehovsmeldingType.SØKNAD -> "Søknad om tilbehør"
            else -> error("Forventet ikke behovsmeldingstype $behovsmeldingType for sak med kun tilbehør.")
        }
}
