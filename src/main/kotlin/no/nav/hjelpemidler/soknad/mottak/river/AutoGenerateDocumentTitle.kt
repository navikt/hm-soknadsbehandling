...
object AutoGenerateDocumentTitle {
    ...
    suspend fun generateTitle(behovsmelding: JsonNode): String {
        ...
        val hmsnrs = when (behovsmeldingType) {
            BehovsmeldingType.BRUKERPASSBYTTE -> setOf(behovsmelding["brukerpassbytte"]["hjelpemiddel"]["artnr"].stringValue())
            else -> {
                val hmHmsnrs = behovsmelding["soknad"]["hjelpemidler"]["hjelpemiddelListe"]
                    .mapNotNull { it["hmsNr"].asString() }
                    .filter { it.isNotEmpty() }.toSet()

                val tilbehørListe = behovsmelding["soknad"]["hjelpemidler"]["tilbehørListe"]
                val tilbehørHmsnrs =
                    tilbehørListe?.mapNotNull { it["hmsnr"].asString() }?.filter { it.isNotEmpty() }?.toSet()
                        ?: emptyList()
        ...
        val produktkategorier = try {
            when (behovsmeldingType) {
                BehovsmeldingType.BRUKERPASSBYTTE -> emptySet()
                else -> {
                    behovsmelding["soknad"]["hjelpemidler"]["produktkategorier"]
                        .mapNotNull { it["produktkategori"]["type"].asString() }
                        .filter { it.isNotEmpty() }.toSet()
                }
            }
        } catch (e: Exception) {
            log.error(e) { "Uthenting av produktkategorier feilet ifbm generering av dokumenttittel" }
            emptySet()
        }

        return generateTitle(hmsnrs, behovsmeldingType, produktkategorier)
    }

    private suspend fun generateTitle(
        hmsnrs: Set<String>,
        behovsmeldingType: BehovsmeldingType,
        produktkategorier: Set<String>
    ): String {
        val produkter = try {
        ...
        val titlerForProduktkategori = produktkategorier.sorted().joinToString(separator = ", ").lowercase()

        val title =
            listOf(titlerForHjelpemidler, titlerForTilbehør, titlerForProduktkategori).filter { it.isNotEmpty() }
                .joinToString(", ")
